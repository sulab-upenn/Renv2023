#This is the same pipeline as run.spectre, however, here we are omitting
#the negative values used in "exp" data frame to get rid of negative expressions
#causing post-normalization challenges for heatmap making


run.spectre.exppositive <- function (phenok,metaFile,markerFile, do.plot, do.summary, do.batchAlign, do.fcsExport,flowType,coFactor,plot.against,ref.ctrls)
  ...
  # this version does not allow clustering with a subset of the markers yet. aka single marker staining plotted=markers used for clustering
{
  ...
  setwd(PrimaryDirectory)
  if (flowType == "cytof"){
       source("read.cytofFiles.R") # Recalls pipeline for Cytof made by Tammy
       source("make.cytofheatmap.R")  # Recalls pipeline for Cytof made by Tammy
  }
  ...
  for(i in transf.cols){
    make.colour.plot(cell.dat, i, plot.against)
  }
  ...
  } else {make.pheatmap(exp,"fastPG_Clusters", plot.cols = cellular.cols,normalise=TRUE, standard.colours = "BuPu")
  }
  
  write.csv(exp, "heatmapvalues.csv")
  
  sifr <- exp[, names(exp) := lapply(.SD, function(x) pmax(x, 0))]
  print(sifr)
  
  write.csv(sifr, "heatmappositive.csv")
  
  
  # 1. Identify numeric columns
  numeric_cols <- names(sifr)[sapply(sifr, is.numeric)]
  
  # 2. Exclude 'fastPG_Clusters' from numeric_cols
  numeric_cols <- setdiff(numeric_cols, "fastPG_Clusters")
  
  # 3. Define a normalization function (e.g., min–max normalization)
  ...
  # 4. Apply normalization only to the selected numeric columns
  sifr[, (numeric_cols) := lapply(.SD, min_max_normalize), .SDcols = numeric_cols]
  ...
  #### standard.colours is for heatmap colors with False fold change,  fold.colours is for colouring the folds.
  #heatmap pdf
  if (flowType == "cytof") {
  ...
  } else {make.pheatmap(sifr,"fastPG_Clusters", plot.cols = cellular.cols,normalise=FALSE, file.name=paste0("PositiveHeatmap.png"), standard.colours = naw)
    make.pheatmap(sifr,"fastPG_Clusters", plot.cols = cellular.cols,normalise=FALSE, file.name=paste0("PositiveHeatmap.pdf"), standard.colours = naw)
  }
  ...
  if (do.plot!=FALSE){make.colour.plot(cell.sub, "UMAP_X", "UMAP_Y", group.col, col.type = 'factor')
    
    make.colour.plot(cell.sub, "UMAP_X", "UMAP_Y", sample.col, col.type = 'factor')
    make.colour.plot(cell.sub, "UMAP_X", "UMAP_Y", donor.col, col.type = 'factor')
    
    make.multi.plot(cell.sub, "UMAP_X", "UMAP_Y", "fastPG_Clusters", group.col, col.type = 'factor',save.each.plot = TRUE)
    make.multi.plot(cell.sub, "UMAP_X", "UMAP_Y", cellular.cols,save.each.plot = TRUE)
    ...
    for(i in cellular.cols){
      ...
      make.multi.plot(cell.sub, "UMAP_X", "UMAP_Y", i, col.min.threshold = percentile, group.col, figure.title = paste0('Multiplot - ', i, '.png'))
    }
